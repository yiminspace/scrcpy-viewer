...
The sidebar's recording history shows saved files with their time and a thumbnail. Click an entry to open it in the system default video player. Hover over an entry to reveal its trash button. It asks for confirmation before moving the video to the system Trash and removing it from history; Cancel is the default action. History loads on launch and after saves from the selected recording folder and files explicitly saved by this app; it does not search other folders. Manually chosen save paths are remembered locally so those recordings remain available after relaunch.
