...
点击工具栏的录屏按钮，选择 MP4 保存位置，完成后点击停止。录屏包含主屏和所有活跃副屏，不受选中屏幕或横向滚动位置影响；新出现的活跃副屏自动加入，休眠或移除的屏幕保留带状态和时间的最后画面。保存成功后，点击窗口底部的「查看录屏」在 Finder 中查看全部分段文件。

输出为无声 H.264 MP4，尺寸跟随画面实际比例，不加黑边；高度最高 720 像素、宽度最高 2560 像素、12 fps，码率随输出尺寸调整，上限 1.6 Mbps。新副屏加入或屏幕旋转导致布局变化时，自动保存下一段 MP4，例如 `screens-part002-<唯一后缀>.mp4`。为控制文件体积，采用 MP4，暂不提供 GIF 导出。切换设备或退出应用时会完成当前录制。
...
侧栏的录屏历史显示保存时间和缩略图，点击条目即可使用系统默认播放器播放。启动应用或录屏保存后，从当前录屏目录和本应用明确保存过的文件加载列表，不扫描其他目录。手动选择的保存路径会记在本机，重启后仍可回看这些录屏。
